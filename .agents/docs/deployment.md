...
AWS 관리형 정책 `AmazonSSMManagedInstanceCore`와 다음 권한을 추가한다. 사용자 관리 KMS 키로
배포 중 생성되는 임시 GHCR 토큰을 읽도록 다음 권한을 추가한다. 사용자 관리 KMS 키로
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          ...
          "token.actions.githubusercontent.com:sub": "repo:tech4good-One-T/aiserver:ref:refs/heads/main"
        }
      }
    }
  ]
